...
get_csoil_star <- function(df){
  df %>%
    ...
    mutate(csoil_star_final = npp_final* (csoil_final/rh_final)) # HY proposed a new equation here for Csoil_star_final, waiting
    # for Beni's reply
}
...
#save(gdf,file="gdf_Trendyv7.rda")
load("gdf_Trendyv7.rda")
```
...
ggsave("dcsoil_star_fveg_XX_trendy_s1_bymodl.pdf", width = 9, height = 7)

```

### dCveg vs. dcveg_ag change
...
modobs <- gdf$data[[4]] %>%
  dplyr::filter(dcveg_ag < 2 & dcveg_ag > -2 & dcveg < 2 & dcveg > -2) %>%
  dplyr::filter(!is.nan(dcveg_ag), !is.nan(dcveg), !is.infinite(dcveg_ag), !is.infinite(dcveg)) %>%
  analyse_modobs2("dcveg_ag", "dcveg", type = "heat", plot_subtitle = FALSE, plot_linmod = FALSE)
modobs$gg +
  xlim(c(-0.5,1)) + ylim(c(-0.5,1)) +
...
gdf %>%
  ...
  dplyr::filter(dcveg_ag>-0.9) %>% # remove points where veg collapses
  dplyr::filter(dcveg_ag < 2 & dcveg_ag > -2 & dcveg < 2 & dcveg > -2) %>%
  dplyr::filter(!is.nan(dcveg_ag), !is.nan(dcveg), !is.infinite(dcveg_ag), !is.infinite(dcveg)) %>%
  ggplot(aes(x = dcveg_ag, y = dcveg)) +
  stat_density_2d(aes(fill = after_stat(nlevel)), geom = "polygon") +
...
ggsave("cveg_dcveg_ag_trendy_s1_bymodl.pdf", width = 9, height = 7)
...
modobs <- gdf$data[[2]] %>%
  ...
  analyse_modobs2("dnpp", "dgpp", type = "heat", plot_subtitle = FALSE, plot_linmod = FALSE)
modobs$gg +
  xlim(c(-0.5,1)) + ylim(c(-0.5,1)) +
...
ggsave("dgpp_dnpp_trendy_s1_bymodl.pdf", width = 9, height = 7)
```
...
ggsave("cveg_dnpp_trendy_s1_bymodl.pdf", width = 9, height = 7)
...
ggsave("csoil_dnpp_trendy_s1_bymodl.pdf", width = 9, height = 7)
```
...
ggsave("csoil_cveg_trendy_s1_bymodl.pdf", width = 9, height = 7)
...
ggsave("csoil_cveg_ag_trendy_s1_bymodl.pdf", width = 7, height = 7)
