...
gg3 <- df_models_trend |> 
  ...
  theme_classic() + 
  scale_color_manual(values = c("black", sample(getPalette(colourCount)))) +
  labs(title = "Trend 1959-2020", 
       x = "Model type and observations", 
       y = expression(paste("Sink trend (g C m"^2, "yr"^-2, ")")))
...
gg4 <- df_models_trend |> 
    mutate(model = factor(model, 
                        levels = (c("Observations", 
                                    df_models_trend$model[-which(df_models_trend$model == "Observations")])))) |>  
  ggplot(aes(x = type, y = iav)) +
  geom_boxplot(fill = "azure3", outlier.color = "white") +
  geom_jitter(aes(color = model), width = 0.2, size = 3) + 
  theme_classic() + 
  scale_color_manual(values = c("black", sample(getPalette(colourCount)))) +
  labs(title = "IAV 1959-2020", 
       x = "Model type and observations", 
       y = expression(paste("IAV of the land sink (g C m"^2, "yr"^-2, ")")))

bottomrow <- cowplot::plot_grid(gg2, gg3, rel_widths = c(1,1.3), ncol = 2, labels = c("b", "c"))
cowplot::plot_grid(gg1, bottomrow, nrow = 2, rel_heights = c(0.8, 1), labels = "a")
